...
import { Lightbulb, Sparkles, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
        ...
        <CardContent className="p-8">
          <div className="flex items-center justify-center space-y-4 flex-col">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">AI is analyzing your resume for improvements</p>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="w-full gradient-card shadow-medium">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">AI-powered suggestions to enhance your resume</p>
            </div>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
          ...
          {['skills', 'experience', 'format', 'keywords'].map(category => (
            <TabsContent key={category} value={category} className="space-y-4 mt-6">
              {suggestions.filter(s => s.category === category).map((suggestion, index) => (
                <Card key={index} className={`border-l-4 ${getPriorityColor(suggestion.priority)}`}>
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start gap-3">
                      <span className="text-2xl">{getCategoryIcon(suggestion.category)}</span>
